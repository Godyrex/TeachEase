...
export class SuperAdminService {
  ...
  toggleBan(email: string) {
    return this.http.get(`${this.baseUrl}/toggle-user-ban`, {params: {email: email}});
  }
  toggleEnable(email: string) {
    return this.http.get(`${this.baseUrl}/toggle-user-enabled`, {params: {email: email}});
  }
  addRole(email: string, role: string) {
    return this.http.get(`${this.baseUrl}/add-role`, {params: {email: email, role: role}});
  }
    removeRole(email: string, role: string) {
        return this.http.get(`${this.baseUrl}/remove-role`, {params: {email: email, role: role}});
    }
}
